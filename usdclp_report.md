# Informe Cuantitativo de Posicionamiento USD/CLP (v5.0)

**Fecha de Ejecución:** `2026-09-24 14:43:20 CLT`  
**Fecha de Datos:** `2026-09-24`

---

## 🎯 Señal de Mercado

| Métrica | Valor |
| :--- | :--- |
| **Señal de Trading** | **`BUY_USD`** |
| **Probabilidad Estimada Alza USD** | **`63.4%`** |
| **Régimen de Mercado (HMM)** | `SYSTEMIC_STRESS` |
| **Precio Actual USD/CLP** | **$961.80 CLP** |
| **Stop-Loss Dinámico (2.0x ATR)** | **$937.21 CLP** |
| **Take-Profit Sugerido (3.5x ATR)** | **$1,004.84 CLP** |
| **Nivel de VIX** | `15.8` |

---

## 📈 Rendimiento Histórico de Simulación (Walk-Forward OOS)

| Métrica de Desempeño | Valor |
| :--- | :--- |
| **Precisión Direccional Promedio** | **`47.1%`** |
| **Rango de DA (Mín / Máx)** | `[11.9%, 66.7%]` |
| **Retorno Acumulado Simulado** | **`+5.50%`** |
| **Retorno Anualizado (CAGR)** | **`+4.01%`** |
| **Sharpe Ratio** | **`0.358`** |
| **Sortino Ratio** | **`0.861`** |
| **Profit Factor** | **`1.20`** |
| **Win Rate en Operaciones** | **`40.5%`** (37 trades) |
| **Máximo Drawdown** | **`-11.31%`** |

---

## 🔍 Factores de Mayor Impacto (Explicabilidad SHAP)

```
us_10y2y                            : +1.139016
macd_line                           : +0.791583
oil                                 : +0.361879
return_21d                          : +0.327257
sma_50                              : +0.324166
hy_spread_change_5d                 : +0.229392
brl                                 : +0.226530
cny_usdclp_corr_21d                 : +0.178579
vol_ratio_21_63                     : +0.174688
return_10d                          : +0.140280
```

---
*Generado automáticamente por el Pipeline Predictor USD/CLP v5.0 Enterprise.*
