# Informe Cuantitativo de Posicionamiento USD/CLP (v5.0)

**Fecha de Ejecución:** `2026-08-27 02:08:07`  
**Fecha de Datos:** `2026-08-13`

---

## 🎯 Señal de Mercado

| Métrica | Valor |
| :--- | :--- |
| **Señal de Trading** | **`BUY_USD`** |
| **Probabilidad Estimada Alza USD** | **`92.9%`** |
| **Régimen de Mercado (HMM)** | `SYSTEMIC_STRESS` |
| **Precio Actual USD/CLP** | **$912.77 CLP** |
| **Stop-Loss Dinámico (2.0x ATR)** | **$897.10 CLP** |
| **Take-Profit Sugerido (3.5x ATR)** | **$940.20 CLP** |
| **Nivel de VIX** | `15.2` |

---

## 📈 Rendimiento Histórico de Simulación (Walk-Forward OOS)

| Métrica de Desempeño | Valor |
| :--- | :--- |
| **Precisión Direccional Promedio** | **`48.7%`** |
| **Rango de DA (Mín / Máx)** | `[12.7%, 82.5%]` |
| **Retorno Acumulado Simulado** | **`+1.80%`** |
| **Retorno Anualizado (CAGR)** | **`+0.26%`** |
| **Sharpe Ratio** | **`0.328`** |
| **Sortino Ratio** | **`0.434`** |
| **Profit Factor** | **`1.18`** |
| **Win Rate en Operaciones** | **`58.3%`** (12 trades) |
| **Máximo Drawdown** | **`-6.14%`** |

---

## 🔍 Factores de Mayor Impacto (Explicabilidad SHAP)

```
us10y                               : +0.677616
macd_signal                         : +0.456904
rsi_14                              : +0.443908
sp500                               : +0.405358
tot_return_21d                      : +0.309078
macd_line                           : +0.277194
atr_14                              : +0.196604
copper_usdclp_corr_21d              : +0.186372
macd_hist                           : +0.148959
realized_vol_63                     : +0.148062
```

---
*Generado automáticamente por el Pipeline Predictor USD/CLP v5.0 Enterprise.*
